 #!/bin/bash

echo "ssh-rsa  melaraj@Manuels-MacBook-Pro.local" >> /home/ec2-user/.ssh/authorized_keys

echo "ssh-rsa  oresteslorda@Orestess-MBP" >> /home/ec2-user/.ssh/authorized_keys

yum update -y
yum install docker -y
service docker start
mkdir -p /opt/consuldata

docker run --name consul2 --restart=always -h consul2 	-v /opt/consuldata:/data -p 8300:8300 	-p 8301:8301 	-p 8301:8301/udp 	-p 8302:8302 	-p 8302:8302/udp 	-p 8400:8400 	-p 8500:8500 	-p 172.17.0.1:53:53 	-p 172.17.0.1:53:53/udp -d 	progrium/consul -server  -join 10.1.1.99

echo "service docker start" >> /etc/rc.local
reboot