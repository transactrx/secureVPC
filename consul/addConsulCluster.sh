...
CONSUL1=$(aws ec2 run-instances --security-group-ids $CONSUL_SG --instance-type $CONSUL_INSTANCE_TYPE --subnet-id $SUBNET1 --private-ip-address 10.101.1.99 --associate-public-ip-address --image-id ami-10ae537d --user-data "$CONSUL1_USER_DATA"|jq -r .Instances[0].InstanceId)
...
CONSUL_ELB_NAME="$VPCNAME"-CONSUL-ELB
