...
CONSUL1=$(aws ec2 run-instances --security-group-ids $CONSUL_SG --instance-type $CONSUL_INSTANCE_TYPE --subnet-id $PUBLIC_SUBNET_1 --private-ip-address 10.1.1.99 --associate-public-ip-address --image-id ami-93e905fe --user-data "$CONSUL1_USER_DATA" -ui-dir /ui|jq -r .Instances[0].InstanceId)
aws ec2 create-tags --resources $CONSUL1  --tags Key=Name,Value="$VPCNAME"_CONSUL1
CONSUL2=$(aws ec2 run-instances --security-group-ids $CONSUL_SG --instance-type $CONSUL_INSTANCE_TYPE --user-data "$CONSUL2_USER_DATA" --private-ip-address 10.1.2.99  --subnet-id $PUBLIC_SUBNET_2 --associate-public-ip-address --image-id ami-93e905fe -ui-dir /ui |jq -r .Instances[0].InstanceId)
aws ec2 create-tags --resources $CONSUL2  --tags Key=Name,Value="$VPCNAME"_CONSUL2
CONSUL3=$(aws ec2 run-instances --security-group-ids $CONSUL_SG --instance-type $CONSUL_INSTANCE_TYPE --user-data "$CONSUL3_USER_DATA" --private-ip-address 10.1.2.100  --subnet-id $PUBLIC_SUBNET_2 --associate-public-ip-address --image-id ami-93e905fe -ui-dir /ui |jq -r .Instances[0].InstanceId)
