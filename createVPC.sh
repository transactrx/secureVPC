...
CONSUL2=$(aws ec2 run-instances --security-group-ids $CONSUL_SG --instance-type t2.micro --user-data "$CONSUL2_USER_DATA" --private-ip-address 10.1.2.99  --subnet-id $PUBLIC_SUBNET_2 --associate-public-ip-address --image-id ami-93e905fe|jq -r .Instances[0].InstanceId)
aws ec2 create-tags --resources $CONSUL2  --tags Key=Name,Value="$VPCNAME"_CONSUL2
CONSUL3=$(aws ec2 run-instances --security-group-ids $CONSUL_SG --instance-type t2.micro --user-data "$CONSUL3_USER_DATA" --private-ip-address 10.1.2.100  --subnet-id $PUBLIC_SUBNET_2 --associate-public-ip-address --image-id ami-93e905fe|jq -r .Instances[0].InstanceId)
