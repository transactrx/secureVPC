...
echo "Creating FreeIPA cluster for VPC =$VPCNAME and DOMAIN = $DOMAIN_NAME" 

SUBNET1=$PRIVATE_SUBNET_1 
SUBNET2=$PRIVATE_SUBNET_2


FREEIPA_SG=$(aws ec2 create-security-group --vpc-id $VPCID --group-name $VPCNAME"-FreeIPA" --description  "$VPCNAME FreeIPA service SG"|jq -r .GroupId)
...
FREEIPA1_USER_DATA=$(cat FreeIPA1_userdata.sh|base64)

FREEIPA_INSTANCE_TYPE=t2.micro 

FREEIPA1=$(aws ec2 run-instances --security-group-ids $CONSUL_SG --instance-type $FREEIPA_INSTANCE_TYPE --subnet-id $SUBNET1 --private-ip-address 10.1.1.99 --associate-public-ip-address --image-id ami-93e905fe --user-data "$FREEIPA1_USER_DATA"|jq -r .Instances[0].InstanceId)
aws ec2 create-tags --resources $FREEIPA1  --tags Key=Name,Value="$VPCNAME"_FREEIPA1
